...
# --- Configuración ---
threshold = 0.5
dir_cat   = "ncats/dataset/predict/cat"
dir_nocat = "ncats/dataset/predict/nocat"

H, W, C = 64, 64, 3
assert test_data.shape[1] == H*W*C, f"Se esperaban {H*W*C} features, llegaron {test_data.shape[1]}"
test_imgs = test_data.reshape(-1, H, W, C)   
probs     = predictions.reshape(-1)           

for i, (img, p) in enumerate(zip(test_imgs, probs), start=1):
    subdir = dir_cat if p > threshold else dir_nocat
    fname = f"img_{i:04d}_p{p:.2f}.png"
    plt.imsave(f"{subdir}/{fname}", img)

print(f"Se guardaron {len(probs)} imágenes en '{dir_cat}' y '{dir_nocat}' (umbral {threshold}).")